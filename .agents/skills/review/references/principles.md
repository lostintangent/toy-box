...
Use this catalog to interpret code and compiler or lint signals during a code review. Each pattern is a signal to investigate, not a finding by itself. Choose a correction only after confirming the intended ownership, lifetime, and behavior through call sites and tests.
...
9. [React Query](#9-keep-server-state-and-commands-in-react-query)
10. [Runtime schemas](#10-make-runtime-schemas-the-source-of-static-types)
...
## 9. Keep server state and commands in React Query

**Look for:** query data copied into local state, ad hoc fetch effects, query inputs absent from the key, request/response writes managed with local pending/error state, mutation observers created in a parent only to distribute callbacks, or custom hooks that merely rename one Query result or return an actions bag.

**Correct by:** rendering authoritative query data directly, subscribing with the narrowest useful projection at the consumer, and applying request/response writes through mutations. Keep reusable server access and cache policy in query or mutation option factories; put the observer in the component or workflow that owns its UI.
...
### Choose the smallest honest abstraction

- Use `queryOptions` to define canonical resource identity, fetching, freshness, and shared cache policy. Call `useQuery` near consumption and use `select` so each observer receives only its semantic dependency.
- Use `mutationOptions` to define a request/response command and its shared cache transition. Call `useMutation` at the closest owner of the action, pending UI, and error presentation instead of prop-drilling an actions facade.
- Use a custom hook when it owns a real React lifetime or resource, client-only state, a multi-step workflow, composition across multiple sources, or a widely reused semantic subscription policy. A hook need not be large, but it must remove decisions or enforce an architectural invariant.
- Avoid a custom hook whose only role is to call one query or mutation, rename fields, provide incidental defaults, or wrap `mutateAsync`. Let consumers select different projections rather than making them share one broad convenience result.

### Model mutation state directly

- Treat `isPending`, `error`, `data`, and `variables` as the operation state. Add local state only when it represents a distinct lifetime, such as form input or waiting for a later canonical stream event.
- Put a true optimistic transition in `onMutate` and roll back that transition in `onError`. Update the cache from the response or invalidate authoritative data when required; do not add resets, repair paths, logging, or invalidation without an observable behavior that needs them.
- Add a mutation key only for shared defaults, cache-wide filtering/observation, or persisted mutation behavior. Use `scope` only when calls must execute serially; neither is required for an ordinary component-owned mutation.
- Default mutating request/response server calls to mutations. Keep streams, subscriptions, and imperative adapter side channels at the boundary that owns their actual lifetime when the mutation lifecycle does not model them.

## 10. Make runtime schemas the source of static types

**Look for:** a TypeScript type that repeats a Zod schema, `satisfies z.ZodType<HandwrittenType>`, casts after successful parsing, equivalent schemas copied across server functions and tools, or runtime validators added to trusted internal values without a distinct trust boundary.

**Correct by:** defining one canonical schema for each untrusted ingress and inferring any named TypeScript type from it. Compose and reuse that schema wherever the same operation is exposed, including browser RPC and agent tools.
...
- Validate values where trust changes: server-function inputs, route input, agent-tool parameters, external service responses, and persisted or historical payloads that the application intentionally treats as untrusted.
- Do not define a named TypeScript type when the schema, handler, or server-function return already provides the required inference.
- Keep trusted server outputs and internal representations as ordinary TypeScript when they need no runtime parsing: UI state and props, mutation workflow variables, reducer state, database row adapters, and framework-inferred server-function results.
- Do not parse the same value again at every layer. Validate once at the owning boundary, then pass the validated domain value inward.
