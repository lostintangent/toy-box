...
| Message  | Purpose                                                                                                                                            |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `init`   | Creates or resumes a PTY for the client, using an initial size (row/cols), and optinonally, a user-specified shell (defined in settings).          |
...
| `close`  | Close the PTY for the client. This happens when the user clicks the `X` button in the integrated terminal, which explicitly terminals the session. |
...
When the server encounters a binary message, it treats it as stdin content and directly writes it to the PTY. Additionally, when the PTY emits output, the server directly writes it to the client's socket as a binary message with no protoco envelope.
...
- Client connects within the orpha timer window, and the server resumes the existing PTY
- Server replays the current scrollback buffer _(see below for details)_

### Client Close

- explicit `close` destroys PTY immediately
- orphan timeout destroys PTY if no client reconnects
- idle timeout destroys long-inactive PTYs
  Why: orphan timeout protects reconnect UX; idle timeout bounds background
  resource usage.

### Server Exit

- when PTY exits, server emits `exit` and removes the session
- stale exit races are ignored by checking the active session identity
- slow clients are protected by buffered-amount backpressure checks
- process restart is a hard recovery boundary; clients must start a new terminal
  session
