...
export function SessionView({
  ...
  // `selectedModel` is the user-scoped default persisted in localStorage.
  // It is the starting choice for drafts, but existing sessions should prefer
  // their own session state instead of falling back to it.
  const defaultModel = selectedModel;

  // Local picker state for a model choice that has not been reflected by the
  // session yet. This covers both a draft's pre-first-message selection and
  // the optimistic UI state after changing the model on an existing session.
  const [pendingModel, setPendingModel] = useState<string | undefined>(undefined);

  // Reset the pending selection when switching to a different session.
  useEffect(() => {
    setPendingModel(undefined);
  }, [sessionId]);

  // Only send a model override when local UI state needs one:
  // - drafts use the default model until the session exists
  // - existing sessions only send an explicit pending selection
  const configuredModel = isDraft ? (pendingModel ?? defaultModel) : pendingModel;
  ...
  }, [isDraft, configuredModel, selectedDirectory, sessionId, onDraftSessionCreated]);
  ...
  // it arrives. Drafts fall back to the user-scoped default until the session
  // exists on the server.
  const sessionModel = sessionData?.model;
  const displayedModel = pendingModel ?? sessionModel ?? (isDraft ? defaultModel : undefined);
