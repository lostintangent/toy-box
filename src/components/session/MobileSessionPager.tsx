...
export function MobileSessionPager({
  ...
  // Single session — render SessionView directly, no pager chrome
  if (sessionIds.length <= 1) {
    return (
      <SessionView
        sessionId={sessionIds[0] ?? selectedSessionId}
        isSessionRunning={streamingSessionIds.includes(sessionIds[0] ?? selectedSessionId)}
        isSessionUnread={unreadSessionIds.includes(sessionIds[0] ?? selectedSessionId)}
        onBack={onBack}
        {...sessionViewProps}
      />
    );
