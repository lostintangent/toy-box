...
export function AutomationScheduleEditor({
  ...
  const [state, setState] = useState(() => initializeScheduleState(value));
  const normalizedSimpleSchedule = normalizeSimpleSchedule(state.simpleSchedule);

  useEffect(() => {
    setState(initializeScheduleState(value));
  }, [idPrefix, value]);
  ...
  function updateMode(mode: ScheduleInputMode) {
    ...
    if (mode === "cron") {
      ...
      onChange(
        simpleScheduleToCron(ensureScheduleMode(normalizedSimpleSchedule, currentSimpleMode)),
      );
      return;
    }

    const parsed = state.mode === "cron" ? cronToSimpleSchedule(value) : normalizedSimpleSchedule;
    ...
    onChange(simpleScheduleToCron(nextSchedule));
  }
