...
type CanvasWorkspacePane = Extract<WorkspacePane, { kind: "canvas" }>;

// Canvas is part of the generalized pane contract but has no title-bar actions,
// so it accepts PaneProps (variant/actionsSlot) and ignores them.
type CanvasPaneProps = PaneProps & { pane: CanvasWorkspacePane; onBack?: () => void };

export function CanvasPane({ pane, onBack }: CanvasPaneProps) {
  const { canvas } = pane;
