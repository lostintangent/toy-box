...
The pane carries one of three modes: `read`, `edit`, or `shared`. Read keeps artifact content presentation-only while still allowing Markdown comments. Edit persists user changes without agent notification. Shared persists changes and notifies the owning session's agent. Ordinary session artifacts open in edit mode, automation artifacts open in read mode, and Inbox artifacts open in shared mode with follow-up conversation through the managed session overlay.

An SDK canvas is not an artifact. It is an SDK-provided URL surface with its own identity and revision, so it does not participate in file read, write, watch, serve, or edit-notification behavior. The built-in SVG drawing surface is distinct: its standard `.svg` file is an ordinary editable artifact and uses the full artifact lifecycle.
...
- HTML renders in a sandboxed iframe. A generated serve base lets relative scripts, styles, images, and links resolve within the source session's artifact storage.
...
Each built-in renderer lives in its own kind directory. The SVG kind tells its story through progressively narrower boundaries:

- `store/` names the complete editor domain layered over a native SVG document. `types` introduces `EditorState` first—read-only policy, viewport, active tool and style defaults, selection, current gesture, and reversible history—then the independently meaningful `Tool`, `Gesture`, `Viewport`, `History`, and `HistoryEntry` values. `store` is the runtime manifestation of that model and exposes its semantic actions, while `viewport` defines their pure navigation transitions.
- `document/` owns durable SVG semantics. `document` loads, mounts, mutates, and publishes one authoritative native SVG tree; `source` parses, validates, and serializes persisted text; `nodes` constructs native SVG nodes and subtrees; `style` projects and changes native presentation attributes; and `history` defines and applies reversible native-DOM entries. Document mutations return history entries but do not own editor history or silently publish themselves.
- `SvgArtifact` binds one artifact file and pane mode to one native document and one editor store, and projects SVG-specific actions into the pane shell. `editor/Editor` is the composition root for the local interactive experience: it connects the document and store to the editor capabilities and visibly orders their layers. There is no parallel scene model or editor-session wrapper.
- `editor/drawing/` owns drawing-specific admission and overlays. Its source resolves the active drawing tool and style defaults; `DrawingCursor` and `TextInsertion` own the two drawing-specific overlays. Drawing needs no React hook, transaction abstraction, parallel draft-SVG layer, or viewport-space copy of its geometry. `editor/selection/` owns selection policy, hit targeting, chrome, text editing, and clipboard commands. Native hit paths resolve in a deliberate order: editable text, the nearest named semantic group, then the deepest renderable SVG element; marquee enumeration honors those same authored-group boundaries. `frame` projects native elements into the shared manipulation frame and derives the exact adaptive handle set used for both rendering and hit testing. `SelectionLayer` owns painting that transient frame, while `transform` captures the selected native elements and translates screen-space movement, resizing, and rotation back into SVG transforms. A target click remains pending until ordinary pointer movement becomes a drag, so text caret placement is browser-native rather than a separate gesture; blank-space dragging selects only elements fully enclosed by the marquee. `editor/viewport/` owns viewport measurement and wheel input, exposes navigation as a gesture source, and renders the dot-grid layer.
- `editor/gestures/` owns the pointer-interaction algebra shared by drawing, selection, and viewport. `gesture` defines source ownership, controllers, outcomes, and routing; `useGesture` offers pointer-down to the eligible source, captures the pointer when that source returns a controller, and routes every subsequent update to that controller through completion or cancellation. The store's `Gesture` is the observable, renderable interaction state; the captured controller is its imperative pointer lifetime. `drawing`, `transform`, `marquee`, and `pan` implement the complete live behavior of each gesture family. Drawing controllers insert provisional native SVG edits and either record the already-applied history entry or roll it back exactly.
- `editor/images/` owns raster-image admission, browser decoding, and the drag-and-drop layer; the store performs the resulting native document insertion. `editor/document/` owns the mounted native SVG lifetime and its document-attached keyboard handlers; those handlers map scoped input onto store actions and commands spanning the editor's capabilities.
- `toolbar/` owns the floating SVG controls and compact pane menu. It projects the editor store and invokes semantic actions; it does not own selection-versus-default style policy.

SVG text stays ordinary `<text>` content. Chromium ignores `contenteditable` on SVG elements themselves, so selecting text enables an HTML editing host around the mounted SVG. The browser then edits the real SVG text nodes in place with native caret placement and preserves their formatting subtree. The host uses ordinary rich editing because `plaintext-only` changes SVG whitespace layout; paste is explicitly reduced to a text node so HTML cannot enter the SVG. While that host owns focus, native browser undo and redo handle its text-editing transactions; leaving the text records the net edit as one reversible Toy Box history entry. Programmatic SVG edits continue to use Toy Box history because direct DOM mutations do not enter the browser's editing undo stack.

Erasing remains standard, background-independent SVG. Each gesture creates a chronological luminance-mask layer around the renderable content that exists when the gesture begins; later drawings remain outside earlier masks. The in-progress black path mutates inside that real mask, so erasing paints immediately and either becomes one history entry or disappears on cancellation. The mask region follows its layer's bounding box, while an inert authored-page rectangle keeps degenerate horizontal or vertical content maskable and a large page-derived reveal rectangle avoids renderer clipping without unstable extreme coordinates. Root-level definitions and metadata stay in place. Reopening needs no scan or editor reconstruction: ordinary SVG parsing restores the persisted masks directly.
