...
Workers for one resolved file execute in order, while different files can progress independently. The worker subsystem owns this admission policy and its started/finished projection. Removing a queued association makes its eventual queue slot a no-op; cancelling admitted work delegates to its race-safe supervisor. Consumers may monitor the worker through the general session-completion API and receive its final assistant response, but ephemeral completion is not retained as history. File workers are always ephemeral, stay out of Inbox and the normal session list, and disappear after finishing; a startup sweep deletes ephemeral workers abandoned by a process restart. Source deletion finishes outstanding associations and recursively tears down its worker tree. The watched file remains the durable result.
...
- [`../../../../hooks/files/useFile.ts`](../../../../hooks/files/useFile.ts) owns client file lifecycle; `EditorPane` composes session-owned operations, and renderers own format-specific editing and interaction.
- [`../../../../functions/fs.ts`](../../../../functions/fs.ts) owns validated filesystem browsing and text read/write RPCs. Watch and serve routes exist because browser-native streaming and relative-resource loading need HTTP transports.
