...
export function ArtifactsList({ sourceSessionId, artifacts = [] }: ArtifactsListProps) {
  const setFocusedPaneId = useSetAtom(useWorkspaceContext().focusedPaneAtom);
  // Stable keys for repeated paths; name and icon are resolved per pill (they depend on the
  // reactive artifact-kind registry).
  const pills = useMemo(() => {
