...
export function WorkersMenu({
  ...
  workers: readonly Pick<Worker, "sessionId" | "name">[];
  onCancelWorker: (workerSessionId: string) => Promise<unknown>;
  variant: PaneVariant;
}) {
...
function WorkerItem({
  ...
  worker: Pick<Worker, "sessionId" | "name">;
  index: number;
  onCancel: (workerSessionId: string) => Promise<unknown>;
}) {
