...
- An app pane presents one durable saved app instance and is not inherently session-backed.
...
Selected sessions, files, and saved apps come from one deduplicated, four-pane-capped route root set. When none is selected, Inbox is the fallback root pane. An active `SessionPane` publishes the linked sessions, artifacts, and canvases revealed by its reduced session state; `InboxPane` can publish one explicitly selected Inbox artifact; an `AppPane` can publish sessions or files through the versioned app capability API. Each workspace surface owns its own browser-local publication graph, keyed by the publishing pane's ID, and supplies it to panes through `WorkspaceSurfaceProvider`. Pure derivation in [`../../lib/workspace/panes.ts`](../../lib/workspace/panes.ts) walks those relationships, prioritizes panes, and produces the visible workspace. This state describes one browser's composition; it is not server workspace state and does not copy session transcripts, Inbox rows, app state, or artifact content.
...
[`layout/HyperSession.tsx`](layout/HyperSession.tsx) is an independent mini-workspace around a managed session. It derives that session's linked panes, gives them a separate focus and publication surface, and hosts them in `WorkspacePager` inside a movable window. Saved apps can also be added as browser-local Hyper roots without changing the Main selection; duplicate app mounts share durable app state while publishing children only into their containing surface. Promotion preserves the transcript and any live runtime while transferring lifecycle management into the normal workspace; a future cold resume resolves the session's standard role.
...
- [`../../hooks/workspace/layout/surface.tsx`](../../hooks/workspace/layout/surface.tsx) owns the Main and Hyper surface bundles and provides their focus and publication state contextually. [`../../hooks/workspace/layout/panePublications.ts`](../../hooks/workspace/layout/panePublications.ts) owns publication graph transitions and artifact display mode.
...
- [`panes/app/AppPane.tsx`](panes/app/AppPane.tsx) composes one saved app's Toy Box host, authored runtime, compiled component, and linked-pane publication. [`../../functions/apps/AGENTS.md`](../../functions/apps/AGENTS.md) owns definition and instance semantics.
- Main and Hyper surfaces have independent root, focus, and publication state. `WorkspaceSurfaceProvider` supplies each surface's app-opening capability, pane graph, visible pane projection, capacity, and artifact auto-focus policy without putting surface identity into pane content. App publication actions consult that host projection, and the app SDK reports only files and sessions that are actually visible.
