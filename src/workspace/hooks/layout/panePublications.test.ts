...
import { INBOX_PANE, createEditorPane } from "@workspace/model/panes";
import { sessionFile } from "@files/model";
import { workspaceSurfaces } from "./surface";
import { createPanePublicationsStore, updateEditorPaneMode } from "./panePublications";

describe("pane publication store", () => {
  test("preserves one publisher's selection when another publisher changes", () => {
    const store = createPanePublicationsStore();
  ...
  test("provides a stable server snapshot without a provider", () => {
    ...
    function PublishedPanesProbe() {
      const paneCount = useSelector(
        store,
        (publications) => publications["missing-publisher"]?.length ?? 0,
      );
  ...
  test("keeps Main and Hyper publications independent", () => {
    const publisherPaneId = "shared-pane";
    const pane = createEditorPane(sessionFile("session-a", "result.md"), "edit");
    onTestFinished(() => {
      workspaceSurfaces.main.panePublications.actions.clearLinkedPanes(publisherPaneId);
      workspaceSurfaces.hyper.panePublications.actions.clearLinkedPanes(publisherPaneId);
    });

    workspaceSurfaces.main.panePublications.actions.publishLinkedPanes(publisherPaneId, [pane]);

    expect(workspaceSurfaces.main.panePublications.get()[publisherPaneId]).toEqual([pane]);
    expect(workspaceSurfaces.hyper.panePublications.get()[publisherPaneId]).toBeUndefined();
  });
});

describe("published pane layout", () => {
