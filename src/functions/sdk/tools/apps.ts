...
const listAppDefinitionsTool = defineTool("list_app_definitions", {
  description:
    "Lists available Toy Box app definitions, including built-ins, state schemas, and defaults.",
  parameters: z.object({}).strict(),
  skipPermission: true,
  handler: async () => {
    const apps = await import("@/functions/apps/index");
...
const listAppsTool = defineTool("list_apps", {
  ...
  skipPermission: true,
  handler: async () => {
    const appLifecycle = await import("@/functions/apps/index");
...
const uninstallAppTool = defineTool("uninstall_app", {
  description: "Deletes an owner-installed app definition that has no saved instances.",
  parameters: appDefinitionInputSchema,
  skipPermission: true,
  handler: async (input) => {
    const apps = await import("@/functions/apps/index");
