...
const TEMPLATE_CONTRACT = [
  ...
  "  • `await Toybox.spawnWorker({ name?, prompt, metadata? })` — spawn a disposable worker for this file. The optional friendly name appears in Toy Box's worker UI; the prompt defines the file-specific work; JSON metadata is returned unchanged in `pendingWorkers`, allowing responsive spinners or placeholders.",
...
const registerEditorKindTool = defineTool("register_editor", {
  description:
    "Registers a custom editor ('kind') so that files with the given extension(s) open in a bespoke rendered view inside Toy Box, instead of the default Markdown/HTML handling. " +
    ...
    "The viewer is a self-contained HTML/JS template you author. It is saved under ~/.toy-box/editors/<name>/ and applies to every session.\n\n" +
    TEMPLATE_CONTRACT,
  parameters: z.object({
    name: z
      .string()
