...
description: Create, edit, install, uninstall, or repair Toy Box apps—saved custom React TSX surfaces that use the public app SDK to manage durable state, react to workspace state, open files, and launch ordinary sessions or app-owned workers. Use for any request to build, distribute, or revise a Toy Box app.
---
...
- The **design system** combines inherited theme tokens, scoped Tailwind, and
  built-in components for common controls, feedback, model selection, and
...
An owner-installed definition contains exactly two files:
...
Use a lowercase ID containing only letters, digits, and hyphens; `toybox-` is
reserved for built-ins. `description` and `icon` are optional. Supported icons
are `Archive`, `ArrowLeft`,
...
Built-in definitions are shipped with Toy Box and cannot be overwritten. If a
requested definition has no owner-installed directory and registration reports
that it is built in, create a fork under a new ID rather than modifying Toy Box
source.
...
To remove an owner-installed definition, call `uninstall_app` with its ID.
Use `list_apps` and `delete_app` to remove every saved instance that uses it
first. Built-in definitions cannot be uninstalled. Do not manually delete
definition directories when these tools are available.
