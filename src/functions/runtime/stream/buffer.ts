// Session stream buffer: bounded replay plus live fan-out for
// canonical SessionEvents. Subscribing registers immediately, before the
// caller starts pulling, so producers can publish synchronously without losing
// the first event.
//
...
export function createSessionStreamBuffer(options: {
  capacity: number;
  onNoSubscribers?: () => void;
}): SessionStreamBuffer {
  const buffer: StampedSessionEvent[] = [];
  const subscribers = new Set<SessionSubscriber>();
  let closed = false;

  function replaySince(afterEventId?: number): SessionEvent[] {
    if (afterEventId === undefined) return [...buffer];
    return buffer.filter((event) => event.eventId > afterEventId);
  }

  function deliverToSubscriber(subscriber: SessionSubscriber, event: SessionEvent): void {
  ...
  function disconnectSubscriber(subscriber: SessionSubscriber, notify = true): void {
    if (subscriber.disconnected) return;
  ...
  return {
    publish(event) {
      ...
      buffer.push(published);
      if (buffer.length > options.capacity) {
        buffer.splice(0, buffer.length - options.capacity);
      }

      for (const subscriber of subscribers) {
        deliverToSubscriber(subscriber, published);
      }
