...
export async function installApp(input: z.output<typeof installAppInputSchema>) {
  ...
  const slug = candidate.definition.title
    ...
    .replace(/-+$/g, "");
  const id =
    input.id ?? ownerAppDefinitionIdSchema.parse(slug || `gist-${files.gistId.slice(0, 16)}`);
