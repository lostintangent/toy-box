...
import { evaluateAppBundle } from "@/components/workspace/panes/app/host/bundle";
...
describe("app compiler", () => {
  ...
  test("compiles every built-in definition through the public SDK", async () => {
    expect(BUILT_IN_APP_DEFINITIONS.length).toBeGreaterThan(0);
    for (const source of BUILT_IN_APP_DEFINITIONS) {
