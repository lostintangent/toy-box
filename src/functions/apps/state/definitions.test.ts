...
describe("app definition registry", () => {
  ...
  test("reserves built-in definition ids without silently shadowing them", async () => {
    const { registry, root } = await createRegistry();
    await writeDefinition(root, "toybox-shadow", {
      tsx: "export default function App() { return null; }",
    });
    const reportError = spyOn(console, "error").mockImplementation(() => {});
    onTestFinished(() => reportError.mockRestore());

    await expect(
      installDefinition(registry, "toybox-install", {
        manifest: manifest({ title: "Shadow" }),
        tsx: "export default function App() { return null; }",
      }),
    ).rejects.toThrow("reserved for built-in apps");
    expect(await registry.list()).toEqual([]);
    expect(reportError).toHaveBeenCalledWith(
      'Skipping invalid app definition "toybox-shadow":',
      expect.any(Error),
    );
  });
