...
let defaultDb: AutomationDatabase | undefined;

async function getDefaultDb(): Promise<AutomationDatabase> {
  if (!defaultDb) {
    defaultDb = await AutomationDatabase.open();
  }
  return defaultDb;
}
...
export async function runAutomation(automationId: string): Promise<{ sessionId: string }> {
  ...
  const session = await dependencies.createSession(sessionId, automation.model, automation.cwd);
