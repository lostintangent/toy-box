...
import { parseBrief, type BriefDocument, type BriefSection } from "./model/index";
import { BriefSectionContent, SectionPanel } from "./sections";

function fixture(): BriefDocument {
  const parsed = parseBrief(
    JSON.stringify({
      title: "Flexible brief",
      sections: [
        {
          id: "overview",
          title: "Markdown",
        ...
        },
        {
          ...
          purpose: "Keep pseudocode contracts and structured exhibits beside the brief.",
          kind: "exhibits",
          ...
          items: [
            ...
            },
            {
              ...
              change: "new",
              description:
                "Lock in the conceptual hierarchy without turning its nodes into brief entities.",
              roots: [
                {
                  name: "Brief document",
                  children: [
...
function section(document: BriefDocument, id: string): BriefSection {
  const section = document.sections.find((candidate) => candidate.id === id);
  if (section) return section;
  throw new Error(`Missing section ${id}`);
}

function renderSection(document: BriefDocument, id: string): string {
...
test("renders pseudocode, tree, URI-backed, and embedded exhibits without flattening them", () => {
  ...
  expect(markup).toContain('aria-label="Domain trees"');
  expect(markup).toContain(">Brief document</span>");
...
test("renders card records with direct inspection, source, and removal", () => {
  ...
  expect(markup).toContain('aria-label="Remove Block from brief"');
});
