...
function firstDecisionOptionRelationship(document: BriefDocument): OptionRelationship {
...
describe("brief schema", () => {
  ...
  test("keeps source-backed findings distinct from the spec they ground", () => {
    ...
    expect(findBriefEntity(document, "finding-shared-owner")).toMatchObject({
      type: "finding",
      section: { id: "research-findings" },
    });
    expect(findBriefEntity(document, "current-rendering-ownership")).toBeUndefined();
  ...
  test("parses plan step status and reference-only document tabs", () => {
    ...
    document.tabs = [
      { title: "Brief", sections: ["decisions", "behavior"] },
    ...
    expect(
      resolveBriefTabs(parsed.value).map((tab) => ({
        title: tab.title,
        sections: tab.sections.map((section) => section.id),
      })),
    ).toEqual([
      { title: "Brief", sections: ["behavior", "decisions"] },
      { title: "Plan", sections: ["implementation"] },
    ]);
  ...
  test("requires completion criteria and reserves intrinsic plan fields", () => {
    const missingDoneWhen = JSON.parse(serializeBrief(plannedFixture()));
  ...
  test("requires tabs to partition top-level sections without addressable IDs", () => {
    ...
    duplicateTitle.tabs![1]!.title = "Brief";
    ...
    const addressedTab = JSON.parse(serializeBrief(valid));
    addressedTab.tabs[0].id = "brief";
    expect(parse(addressedTab)).toMatchObject({ ok: false });

    const pendingStatus = JSON.parse(serializeBrief(valid));
  ...
  test("requires globally unique brief entity IDs", () => {
    ...
    expect(parse(duplicateOptionExhibit)).toMatchObject({
      ok: false,
      error: expect.stringContaining("Brief entity IDs must be unique"),
    });
  ...
  test("resolves ID-only references to typed entities", () => {
    const document = plannedFixture();
    expect(findBriefEntity(document, "changed-behavior")).toMatchObject({
      type: "record",
  ...
  test("parses pseudocode definitions as brief entities", () => {
    ...
    expect(findBriefEntity(document, "body-declaration")).toMatchObject({
  ...
  test("parses multiple file-tree roots as one tree exhibit requirement", () => {
    ...
    section.items.push({
      ...
      source: "src/features/briefs/BriefEditor.tsx",
      roots: [
        {
          kind: "folder",
          ...
          children: [
            ...
            { kind: "file", name: "BriefEditor.tsx", change: "modified" },
          ],
        },
        {
          kind: "folder",
    ...
    expect(findBriefEntity(parsed.value, "target-files")).toMatchObject({
      type: "exhibit",
  ...
  test("parses a domain tree as one exhibit without promoting its concepts to entities", () => {
    ...
    expect(parsed).toMatchObject({ ok: true });
    if (!parsed.ok) throw new Error(parsed.error);
    expect(findBriefEntity(parsed.value, "brief-domain")).toMatchObject({
      type: "exhibit",
      label: "Brief document domain",
      change: "new",
    });
    expect(findBriefEntity(parsed.value, "Spec")).toBeUndefined();
    expect(specState(parsed.value).requirements.map((entity) => entity.id)).toContain(
      "brief-domain",
    );
  });
  ...
  test("requires tree roots, supported node changes, and unique sibling names", () => {
    ...
    const invalidChange = JSON.parse(serializeBrief(document));
    ...
    const missingRoot = JSON.parse(serializeBrief(document));
  ...
  test("requires image alternative text", () => {
    const document = JSON.parse(serializeBrief(exhibitsFixture()));
  ...
  test("keeps the bundled worked example valid", () => {
    const parsed = parseBrief(exampleBrief);
    expect(parsed).toMatchObject({ ok: true });
    if (!parsed.ok) throw new Error(parsed.error);
  ...
  test("serializes parsed defaults deterministically", () => {
    ...
    const serialized = serializeBrief(document);
    expect(serialized.endsWith("\n")).toBe(true);
    expect(parseBrief(serialized)).toEqual({ ok: true, value: document });
  });
});
