...
describe("brief transitions", () => {
  ...
  test("edits shared and option-owned records without changing brief identity", () => {
    const document = fixture();
    ...
    expect(findBriefEntity(editedShared, "ordinary-tools")).toMatchObject({
  ...
  test("edits an exhibit without changing its brief identity", () => {
    ...
    const entity = findBriefEntity(edited, "body-declaration");
  ...
  test("edits an option-owned exhibit at its authoritative decision option", () => {
    ...
    expect(findBriefEntity(edited, "durable-state-preview")).toMatchObject({
  ...
  test("does not reinterpret an exhibit's stable identity as another form", () => {
    ...
    expect(
      updateExhibit(document, "target-files", {
        ...
        type: "domain",
        change: "new",
        roots: [{ name: "Brief document", children: [] }],
      }),
    ).toBe(document);
  });
  ...
  test("edits a plan step without changing its implementation links or identity", () => {
    ...
    const entity = findBriefEntity(edited, "foundation-step");
  ...
  test("edits a step inside a named phase while preserving its phase identity", () => {
    ...
    const entity = findBriefEntity(edited, "integration-step");
  ...
  test("lets an editor explicitly correct plan step status", () => {
    ...
    const completedStep = findBriefEntity(completed, step.id);
    ...
    const resetStep = findBriefEntity(reset, step.id);
  ...
  test("removes option exhibits and their now-orphaned plan steps with the decision section", () => {
    ...
    expect(findBriefEntity(removed, "foundation-step")).toBeUndefined();
    expect(findBriefEntity(removed, "integration-step")).toBeDefined();
    expect(parse(removed)).toMatchObject({ ok: true });
  });
