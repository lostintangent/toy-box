...
/**
 * Shared Brief fixtures: one exploratory document covering every section
...
export function parse(value: unknown) {
  return parseBrief(JSON.stringify(value));
}

export function fixture(): BriefDocument {
  const parsed = parse(fixtureInput());
  if (!parsed.ok) throw new Error(parsed.error);
  return parsed.value;
}

export function groundedFixture(): BriefDocument {
...
export function exhibitsFixture(): BriefDocument {
...
export function plannedFixture(): BriefDocument {
  const parsed = parse({
...
export function optionExhibitsFixture(): BriefDocument {
...
export function phasedPlanFixture(): BriefDocument {
  const document = plannedFixture();
  const executionPlan = plan(document, "implementation");
  if (!("steps" in executionPlan)) throw new Error("Expected flat plan fixture");
...
export function plan(document: BriefDocument, id: string): PlanSection {
...
export function flowExhibit(document: BriefDocument, id: string): FlowExhibit {
