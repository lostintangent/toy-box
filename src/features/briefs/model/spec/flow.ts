...
export function flowGraph(document: BriefDocument, flow: FlowExhibit): FlowGraph {
  const index = buildBriefIndex(document.sections);
  const sharedEntities = new Map(briefEntitiesFrom(index).map((entity) => [entity.id, entity]));
...
function entityHasChange(entity: BriefEntity): entity is Extract<BriefEntity, { change: Change }> {
