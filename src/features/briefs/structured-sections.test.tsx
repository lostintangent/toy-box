...
import { BriefExhibitEditor } from "./inspector/ExhibitEditor";
import { PlanStepEditor } from "./inspector/PlanStepEditor";
import { BriefRecordEditor } from "./inspector/RecordEditor";
import { BriefFlowExhibit, BriefPlanSection } from "./sections";
...
function fixture(): BriefDocument {
...
function executionPlan(document: BriefDocument) {
...
function planProjection(document: BriefDocument) {
...
function phasedPlanFixture(): BriefDocument {
  ...
  const parsed = parseBrief(JSON.stringify(document));
  if (!parsed.ok) throw new Error(parsed.error);
  return parsed.value;
}
...
test("renders an authored flow as selectable paths over one shared staged graph", () => {
  const markup = renderToStaticMarkup(
    <BriefFlowExhibit document={fixture()} exhibit={RESTORATION_FLOW} onInspect={() => {}} />,
  );
...
test("builds an exhibit editor without flattening pseudocode definitions", () => {
  const document = fixture();
  const entity = findBriefEntity(document, "restore-contract");
  ...
  expect(domainMarkup).toContain('value="Brief document"');
...
test("keeps unplanned requirements visible when no plan step implements the current spec", () => {
  ...
  const markup = renderToStaticMarkup(
    <BriefPlanSection {...planProjection(document)} section={plan} onInspect={() => {}} />,
  );
