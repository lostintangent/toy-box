...
`BriefDocument`. The runtime Zod schema remains the validating source of truth;
...
is preserved. `EntityId` denotes a globally unique brief entity ID.
...
surface beside the brief that authorized it.
...
Brief entity IDs are globally unique across sections, findings, finding
...
shared brief entities without copying them and may add local waypoints whose
meaning exists only inside the flow.

```jsonc
FlowNode =
  | { "entity": "shared brief entity ID" }
  | {
...
A relative URI resolves from the brief file's directory. An absolute URI must
...
whose experience belongs in the brief without prescribing production
...
from the brief file's directory. Both variants render through Toy Box's
...
- Preserve globally unique brief entity IDs and the separate document-wide
