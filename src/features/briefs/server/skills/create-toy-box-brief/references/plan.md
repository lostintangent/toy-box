...
Read this reference only when a `.brief` document contains or needs a plan, or
...
PlanStep = {
  ...
  "implements": ["eligible brief entity ID", "..."],
...
phases. Step IDs join the document-wide brief entity namespace. Each step has
...
A phase is a named grouping with an ID local to its section. It is not a brief
...
implementation links must remain coherent. Use `execute-toy-box-brief` for
