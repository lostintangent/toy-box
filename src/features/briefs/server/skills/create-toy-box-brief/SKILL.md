---
name: create-toy-box-brief
description: Create, revise, or validate strict Toy Box `.brief` documents, especially when a consequential change benefits from source-grounded findings, explicit decisions, multimodal exhibits, or an executable plan; perform focused editor actions. Follow applicable repository and agent-harness guidance for investigation, design review, and planning; this skill owns how their results are represented and related. Use `execute-toy-box-brief` for implementation or completed-outcome review.
---

# Author Toy Box Brief Documents

Use a brief to help a reader form a strong mental model of a consequential
...
The `.brief` file is durable authored truth: findings ground the change,
...
executable brief plan.

## What makes a great brief

Treat a brief as a readable document first and a semantic model second.
A teammate should understand the central change from its title and open sections
without opening an inspector, following provenance, or learning the brief
ontology.
...
- Read [references/example.brief](references/example.brief) only when the
...
work with the brief plan contract.
...
to a concise `.brief` path in the current session's artifacts folder, and never
