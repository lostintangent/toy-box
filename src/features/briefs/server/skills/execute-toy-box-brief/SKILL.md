...
# Execute and Review a Toy Box Brief Plan

Execute the authored plan or review its completed outcome; do not silently
redesign it. The brief is durable truth for the agreed spec, execution order,
completion conditions, and status. Outcome review is an optional action, not a
stored lifecycle state.

Before changing code or the brief:

1. Read the complete `.brief` file.
2. Read the authoring skill's complete
   [schema reference](../create-toy-box-brief/references/schema.md), including
   its exhibit shapes, and the
   [plan contract](../create-toy-box-brief/references/plan.md) completely.
...
Stop and return the issue to the brief when:
...
1. Reread the latest brief and merge any unrelated human or worker edits before
   writing status.
...
- When the document has no tabs, create `Brief` containing every pre-existing
...
Reread and validate the final brief, confirm every step is complete, and never
