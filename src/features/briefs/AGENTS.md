...
This feature owns the `.brief` object model, editor, and agent workflows. A `BriefDocument`
...
least two uniquely titled tabs; tabs have no ID or brief identity; every
...
groups of ordered steps. Phases are grouping-only values, not brief entities,
...
and worked example in `src/features/briefs/server/skills/create-toy-box-brief/`.
When plan execution changes, update the separate
`src/features/briefs/server/skills/execute-toy-box-brief/` skill as well.
