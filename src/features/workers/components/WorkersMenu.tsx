...
export function WorkersMenu({
  ...
  workers: readonly Extract<Worker, { type: "file" | "app" }>[];
  variant: PaneVariant;
}) {
  if (workers.length === 0) return null;

  const label = `${workers.length} active worker${workers.length === 1 ? "" : "s"}`;
  const trigger =
    variant === "normal" ? (
      <button
        type="button"
...
function WorkerItem({
  ...
  worker: Extract<Worker, { type: "file" | "app" }>;
  index: number;
}) {
  const cancelMutation = useMutation(workerMutations.cancel);
  ...
  function handleCancel() {
    ...
    cancelMutation.mutate(
      worker.type === "file"
        ? { type: "file", file: worker.file, workerSessionId: worker.sessionId }
        : { type: "app", appId: worker.appId, workerSessionId: worker.sessionId },
    );
  }
