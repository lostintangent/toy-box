...
export function ChannelTranscript({
  ...
  const messageGroups = groupAdjacentMembershipMessages(messages);
...
function groupAdjacentMembershipMessages(messages: readonly ChannelMessage[]): ChannelMessage[][] {
  const groups: ChannelMessage[][] = [];
  for (const message of messages) {
    const previous = groups.at(-1);
    const type = membershipMessageType(message);
    if (type && previous && membershipMessageType(previous[0]!) === type) {
...
function membershipMessageType(message: ChannelMessage) {
  if (!isChannelSystemMessage(message)) return undefined;
  return message.content.type === "member_joined" || message.content.type === "member_left"
