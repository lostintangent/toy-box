...
type SimpleScheduleKind = SimpleSchedule["kind"];
...
export function AutomationScheduleEditor({
  ...
  const [state, setState] = useState(() => createScheduleState(value));
  ...
  function updateSimpleSchedule(schedule: SimpleSchedule) {
    setState((current) => ({ ...current, simpleSchedule: schedule, modeHint: null }));
    onChange(simpleScheduleToCron(schedule));
  }

  function updateScheduleForMode(
    mode: SimpleScheduleKind,
      ...
      {state.mode === "daily" ? (
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            value={value}
            onChange={(event) => {
              setState((current) => ({ ...current, modeHint: null }));
...
function ensureScheduleMode(schedule: SimpleSchedule, mode: SimpleScheduleKind): SimpleSchedule {
...
function createScheduleState(cron: string): ScheduleState {
...
function getScheduleModeTransition(
  ...
  cron: string,
  mode: ScheduleInputMode,
): { state: ScheduleState; cron: string } | null {
