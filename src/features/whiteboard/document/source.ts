...
export function validateSvgRoot(root: Element): string | null {
  ...
  for (const element of elements) {
    ...
    if (BLOCKED_ELEMENTS.has(name) || element.namespaceURI === HTML_NAMESPACE) {
      return `The <${element.localName}> element is not supported in editable SVG documents.`;
    }
    ...
    for (const attribute of Array.from(element.attributes)) {
      ...
      if (attributeName.startsWith("on")) {
        return `The ${attribute.name} event attribute is not supported in editable SVG documents.`;
      }
...
function validateCss(css: string): string | null {
  if (BLOCKED_CSS_PATTERN.test(css)) {
    return "Executable or imported CSS is not supported in editable SVG documents.";
  }
...
function validateResourceReference(reference: string): string | null {
  ...
  return `The ${protocol}: resource protocol is not supported in editable SVG documents.`;
}
