...
describe("SVG node construction", () => {
  ...
  test("creates native shapes and definition candidates without mutating the document tree", () => {
    ...
    expect(arrow.element.getAttribute("marker-end")).toBe("url(#whiteboard-arrow)");
    ...
    expect(box.element.getAttribute("fill")).toBe("url(#whiteboard-fill-dots-38bdf8)");
  ...
  test("creates a self-sizing eraser mask with stable page geometry", () => {
    ...
    expect(nodes.path.getAttribute("data-whiteboard-eraser-path")).toBe("");
    expect(nodes.path.getAttribute("d")).toBe("M20 30");
    expect(nodes.maskedContent.getAttribute("data-whiteboard-eraser-layer")).toBe("");
    expect(nodes.mask.children[0]?.getAttribute("x")).toBe("-800000");
    expect(nodes.mask.children[0]?.getAttribute("width")).toBe("1600800");
    expect(nodes.maskedContent.children[0]?.getAttribute("data-whiteboard-eraser-bounds")).toBe("");
