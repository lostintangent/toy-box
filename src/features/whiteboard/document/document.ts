...
export class SvgDocument {
  ...
  mount(host: HTMLDivElement, baseUri?: string): void {
    ...
    this.#mountStyle.textContent = `
      :host, [data-whiteboard-svg-editing-host] {
        display: block;
        width: 100%;
        height: 100%;
        overflow: visible;
  ...
  publishSource(): boolean {
    ...
    if ("error" in serialized) {
      console.error("Unable to save SVG document:", serialized.error);
      return false;
    }
    this.#publish({
      ...this.#snapshot,
...
function isSelectableElement(element: Element): boolean {
  return (
    ...
    !element.hasAttribute("data-whiteboard-eraser-bounds") &&
...
function isSemanticGroup(element: Element): boolean {
  ...
  return Boolean(
    ...
    element.getAttribute("data-whiteboard-selectable"),
  );
}
