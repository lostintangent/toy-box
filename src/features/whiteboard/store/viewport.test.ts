...
describe("SVG editor viewport", () => {
  ...
  test("scales and translates around a moving viewport point", () => {
    const viewport = {
      ...DEFAULT_VIEWPORT,
      zoom: 2,
      panX: -10,
    ...
    const transformed = { ...viewport, ...transformViewport(viewport, 1.5, from, to) };

    expect(transformed.zoom).toBe(3);
    expect(toDocumentPoint(transformed, to)).toEqual(documentPoint);
  });
