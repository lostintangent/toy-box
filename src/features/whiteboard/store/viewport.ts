...
export function zoomViewport(
  ...
  return transformViewport(viewport, factor, viewportPoint, viewportPoint);
}

/** Scales the viewport while moving the document point under `from` to `to`. */
export function transformViewport(
  ...
  const zoom = Math.max(MIN_ZOOM, Math.min(MAX_ZOOM, viewport.zoom * scale));
  const documentPoint = toDocumentPoint(viewport, from);
  return {
    zoom,
    panX: to.x / zoom - documentPoint.x,
    panY: to.y / zoom - documentPoint.y,
  };
}
