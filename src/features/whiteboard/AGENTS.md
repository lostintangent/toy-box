...
This feature presents standard SVG content as a native browser-rendered whiteboard with a transient editor layered around it. It does not translate SVG into a proprietary scene graph. The parsed SVG DOM is the authoritative editable document, while the feature owns the tools, selection, viewport, gestures, and history that make that document interactive.

The feature is host-neutral. Its public contract is current content, editability, relative-resource resolution, content changes, and an optional semantic action projection. It does not import another application feature. A host decides how to persist content and where to present projected actions.
...
- `Whiteboard` binds the host-neutral content contract to one document and store and projects semantic actions through an optional render callback.
...
- `toolbar/` projects store state into the floating in-editor toolbar.
- [`../files/components/editor/kinds/svg/`](../files/components/editor/kinds/svg/) is the Files adapter. It maps file content, mode, persistence, and resource resolution into the editor contract and renders projected actions in pane chrome.
