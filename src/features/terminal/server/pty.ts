...
type TerminalPeer = Pick<Peer, "id" | "bufferedAmount" | "close" | "send" | "websocket">;

interface PTYSession {
  proc: PTYProcess;
  connections: Map<string, TerminalPeer>;
  lastActivity: number;
...
export class PTYManager {
  ...
  public handleInit(peer: TerminalPeer, clientId: string, cols = 80, rows = 24, shell?: string) {
    const session = this.sessions.get(clientId);
    const peerId = peer.id;

    if (!session) {
      if (this.sessions.size >= this.maxSessions) {
        peer.close(undefined, "max-sessions");
        return;
      }

      const newSession = this.createSession(clientId, peer, cols, rows, shell);

      this.sessions.set(clientId, newSession);
      peer.send(JSON.stringify({ type: "ready", resumed: false }));
    } else {
      let resizedOnAttach = false;
      const replacedConnections: TerminalPeer[] = [];
      for (const [existingId, existingPeer] of session.connections) {
        if (existingId !== peerId) {
          replacedConnections.push(existingPeer);
        }
      }
      ...
      session.connections.set(peerId, peer);

      session.orphanClose.cancel();
      this.markSessionActive(session);

      for (const existingPeer of replacedConnections) {
        existingPeer.close(WS_CLOSE_CODE_REPLACED, "replaced");
      }
      ...
      peer.send(JSON.stringify({ type: "ready", resumed: true }));

      if (session.scrollback.isAlternateMode()) {
        session.scrollback.replayModeState((chunk) => peer.send(chunk));
        ...
        session.scrollback.replay((chunk) => peer.send(chunk));
      }
    }
  }
  ...
  public handleDisconnect(clientId: string, peerId: string) {
    const session = this.sessions.get(clientId);
    if (!session) return;
  ...
  private broadcast(clientId: string, payload: string | Uint8Array) {
    ...
    for (const peer of session.connections.values()) {
      try {
        // Check backpressure - skip if client buffer is too large
        if (
          peer.websocket.readyState === WebSocket.OPEN &&
          peer.bufferedAmount < MAX_WS_BUFFERED_BYTES
        ) {
          peer.send(payload);
        }
      } catch (err) {
