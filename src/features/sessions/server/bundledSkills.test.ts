...
describe("bundled SDK skills", () => {
  test("overlapping startup installations preserve the latest complete skill tree", async () => {
    const root = await mkdtemp(join(tmpdir(), "toy-box-skills-"));
    temporaryRoots.push(root);
    await Promise.all([
      installBundledSkills(
        { "/skills/owned/SKILL.md": "First", "/skills/owned/references/old.md": "Old" },
        root,
      ),
      installBundledSkills(
        { "/skills/owned/SKILL.md": "Second", "/skills/owned/references/new.md": "New" },
        root,
      ),
    ]);
    expect(await readFile(join(root, "owned/SKILL.md"), "utf8")).toBe("Second");
    expect(await readFile(join(root, "owned/references/new.md"), "utf8")).toBe("New");
    expect(await Bun.file(join(root, "owned/references/old.md")).exists()).toBe(false);
  });
