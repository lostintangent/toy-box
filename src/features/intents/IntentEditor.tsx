...
import { intentActionKey, type IntentAction } from "./actions";
import {
  findFindingsSection,
  findRecordsSection,
  findIntentEntity,
...
export type IntentEditorProps = {
  ...
  compact: boolean;
  baseUri?: string;
  pendingActions: readonly IntentAction[];
  onContentChange: (content: string) => void;
  onRunAction?: (request: IntentAction) => Promise<void>;
};
...
export function IntentEditor({
  ...
  const actionMutation = useMutation({
    mutationFn: (request: IntentAction) => {
      if (!onRunAction) {
        return Promise.reject(new Error("Intent actions aren't available."));
      }
  ...
  const pending = new Set(pendingActions.map(intentActionKey));
  ...
  function persist(next: IntentDocument) {
    ...
    onContentChange(serializeIntent(next));
  }
  ...
  function dispatch(request: IntentAction) {
    const key = intentActionKey(request);
